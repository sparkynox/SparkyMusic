    ...
    AddToPlaylistDialogOnline(
        ...
        songs = remember { 
            songSelection.map { song ->
                ...
                val metadata = song.toMediaMetadata()
                lumi.sparkynox.sparkymusic.db.entities.Song(
                    song = lumi.sparkynox.sparkymusic.db.entities.SongEntity(
                        id = metadata.id,
                    ...
                    artists = metadata.artists.map { artist ->
                        lumi.sparkynox.sparkymusic.db.entities.ArtistEntity(
                            id = artist.id ?: "",
                            name = artist.name
                        )
                    },
                    album = metadata.album?.let { album ->
                        lumi.sparkynox.sparkymusic.db.entities.AlbumEntity(
                            id = album.id,
                            title = album.title,
                            thumbnailUrl = metadata.thumbnailUrl, 
                            songCount = 0,
                            ...
                            database.transaction {
                                songSelection.forEach { song ->
                                    ...
                                    if ((!allLiked && !metadata.liked) || allLiked) {
                                        ...
                                        val songEntity = lumi.sparkynox.sparkymusic.db.entities.SongEntity(
                                            id = metadata.id,
                                            title = metadata.title,
                                            duration = metadata.duration,
                                            thumbnailUrl = metadata.thumbnailUrl,
