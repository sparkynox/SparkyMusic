...
// A stale google-services.json (wrong package, e.g. left over from the original app this was
// forked from) makes the Firebase plugin fail outright rather than just skipping — so check the
// package inside the file matches, not just that some file is present.
val hasGoogleServicesConfig = file("google-services.json").let { f ->
    f.exists() && f.readText().contains("\"package_name\": \"lumi.sparkynox.sparkymusic\"")
}
