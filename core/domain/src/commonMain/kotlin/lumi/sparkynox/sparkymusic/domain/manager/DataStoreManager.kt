package lumi.sparkynox.sparkymusic.domain.manager
...
interface DataStoreManager {
    ...
    /**
     * Volume booster gain in dB, 0 to 12. Independent of [equalizerPreamp] — applied after the
     * equalizer chain with its own limiter, not through the preamp that only ever cuts.
     */
    val volumeBoost: Flow<Float>

    suspend fun setVolumeBoost(boostDb: Float)

    /**
     * Whether the first-launch welcome screen (offering to log in before landing on Home) has
     * already been shown. Set the first time it's dismissed, either by a login attempt or
     * "Not Now" — never shown a second time, since the same login options stay reachable from
     * Settings afterwards.
     */
    ...
    /**
     * Low-end / "Lite" mode: trims the app's most GPU-hungry visuals — blur/haze effects and
     * crossfade transition animations — for smoother playback on weaker hardware. Everything it
     * touches is purely visual; no feature becomes unavailable, screens just render simpler.
     */
    val performanceMode: Flow<String>

    suspend fun setPerformanceMode(enable: Boolean)
    ...
    companion object Values {
        ...
        const val NOW_PLAYING_STYLE_M3_EXPRESSIVE = "M3 Expressive"
        const val NOW_PLAYING_STYLE_APPLE_MUSIC = "Apple Music"
        const val LYRICS_STYLE_CLASSIC = "Classic"
        const val LYRICS_STYLE_APPLE_MUSIC = "Apple Music"
