package lumi.sparkynox.sparkymusic.domain.mediaservice.player
...
interface MediaPlayerInterface {
    ...
    /**
     * Flat gain boost in dB, on top of whatever the equalizer/crossfade/sleep-fade chain already
     * produces. Independent of [setEqualizer]'s preamp, which only ever cuts.
     *
     * Values above roughly 12 dB stop being "louder" and start being distortion even with a
     * limiter in the way, so implementations that can clip are expected to coerce into a sane
     * range themselves rather than trusting the caller.
     *
     * Default no-op so a backend without a boost stage simply ignores it.
     */
    fun setVolumeBoost(boostDb: Float) = Unit
