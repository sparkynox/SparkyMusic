package lumi.sparkynox.sparkymusic.data.dataStore
...
import lumi.sparkynox.sparkymusic.domain.data.model.network.ProxyConfiguration
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.AI_PROVIDER_GEMINI
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.FALSE
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.GITHUB
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.LOCAL_PLAYLIST_FILTER_OLDER_FIRST
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.PROXY_TYPE_HTTP
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.PROXY_TYPE_SOCKS
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.REPEAT_ALL
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.REPEAT_MODE_OFF
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.REPEAT_ONE
import lumi.sparkynox.sparkymusic.domain.manager.DataStoreManager.Values.SIMPMUSIC
...
import lumi.sparkynox.sparkymusic.common.QUALITY as COMMON_QUALITY
...
internal class DataStoreManagerImpl(
    ...
    override val volumeBoost: Flow<Float> =
        settingsDataStore.data.map { preferences ->
            preferences[VOLUME_BOOST]?.toFloatOrNull() ?: 0f
        }

    override suspend fun setVolumeBoost(boostDb: Float) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[VOLUME_BOOST] = boostDb.toString()
            }
        }
    }

    override val hasSeenWelcomeScreen: Flow<Boolean> =
        settingsDataStore.data.map { preferences ->
            preferences[HAS_SEEN_WELCOME_SCREEN] == TRUE
        }

    override suspend fun setHasSeenWelcomeScreen(seen: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[HAS_SEEN_WELCOME_SCREEN] = if (seen) TRUE else FALSE
            }
        }
    }
    ...
    override val themeColorSource =
        settingsDataStore.data.map { preferences ->
            preferences[THEME_COLOR_SOURCE] ?: DataStoreManager.THEME_COLOR_WALLPAPER
        }
    ...
    override val lyricsStyle = settingsDataStore.data.map { preferences ->
    ...
    override val performanceMode: Flow<String>
        get() =
            settingsDataStore.data.map { preferences ->
                preferences[PERFORMANCE_MODE] ?: FALSE
            }

    override suspend fun setPerformanceMode(enable: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[PERFORMANCE_MODE] = if (enable) TRUE else FALSE
            }
        }
    }
    ...
    companion object Settings {
        ...
        val VOLUME_BOOST = stringPreferencesKey("volume_boost")
        val HAS_SEEN_WELCOME_SCREEN = stringPreferencesKey("has_seen_welcome_screen")
