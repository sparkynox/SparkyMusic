package lumi.sparkynox.sparkymusic.media3.exoplayer
...
import lumi.sparkynox.sparkymusic.media3.audio.BiquadFilter
import lumi.sparkynox.sparkymusic.media3.audio.CrossfadeFilterAudioProcessor
import lumi.sparkynox.sparkymusic.media3.audio.EqualizerAudioProcessor
import lumi.sparkynox.sparkymusic.media3.audio.EqualizerCurve
import lumi.sparkynox.sparkymusic.media3.audio.SleepFadeAudioProcessor
import lumi.sparkynox.sparkymusic.media3.audio.VOLUME_BOOST_MAX_DB
import lumi.sparkynox.sparkymusic.media3.audio.VolumeBoostAudioProcessor
import lumi.sparkynox.sparkymusic.media3.exoplayer.CrossfadeExoPlayerAdapter.Companion.SPEED_PITCH_STEP
import lumi.sparkynox.sparkymusic.media3.service.mediasourcefactory.MergingMediaSourceFactory
 ...
 *
 * Architecture mirrors [lumi.sparkynox.sparkymusic.media_jvm.GstreamerPlayerAdapter]:
...
internal class CrossfadeExoPlayerAdapter(
    ...
    /**
     * Flat gain boost in dB, applied after the equalizer/crossfade/sleep-fade chain by every
     * player's [VolumeBoostAudioProcessor]. Same one-write-covers-every-player shape as
     * [internalEqualizerCurve] and [internalSleepFadeFactor].
     */
    @Volatile
    private var internalVolumeBoostDb = 0f
    ...
    private fun createExoPlayerInstance(): PlayerWithFilter {
        ...
        val volumeBoost = VolumeBoostAudioProcessor { internalVolumeBoostDb }
                    ...
                    DefaultAudioSink
                        ...
                        .setAudioProcessorChain(
                            DefaultAudioSink.DefaultAudioProcessorChain(
                                ...
                                // Volume boost sits last in this group: it acts on the fully mixed
                                // signal those three stages produce, with its own soft limiter as
                                // the final guard against clipping before SilenceSkipping/Sonic.
                                arrayOf(equalizer, crossfadeFilter, sleepFade, volumeBoost),
    ...
    /**
     * Coerced here rather than trusting the caller, per the interface's note — a bogus value
     * reaching [VolumeBoostAudioProcessor] would otherwise be free to push well past where the
     * limiter can still make it sound clean.
     */
    override fun setVolumeBoost(boostDb: Float) {
        internalVolumeBoostDb = boostDb.coerceIn(0f, VOLUME_BOOST_MAX_DB)
    }
                        ...
                        if (retryCount < maxRetryCount) {
                            ...
                            coroutineScope.launch {
                                try {
                                    ...
                                    streamRepository.invalidateFormat("${lumi.sparkynox.sparkymusic.common.MERGING_DATA_TYPE.VIDEO}$currentVideoId")
